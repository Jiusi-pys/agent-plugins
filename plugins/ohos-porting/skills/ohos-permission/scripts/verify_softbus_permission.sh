...
echo -n "[3/6] 配置格式 (纯数组)... "
FIRST_CHAR=$("$HDC_BIN" -t "$DEVICE_ID" shell "head -c 10 $TARGET_PATH" | grep -o '[^[:space:]]' | head -1)
if [[ "$FIRST_CHAR" == "[" ]]; then
    check_pass "正确 (以 [ 开头)"
else
...
echo -n "[4/6] 文件权限... "
PERM=$("$HDC_BIN" -t "$DEVICE_ID" shell "stat -c %a $TARGET_PATH 2>/dev/null || ls -la $TARGET_PATH | awk '{print \$1}'" | tr -d '[:space:]')
if [[ "$PERM" == *"644"* ]] || [[ "$PERM" == *"rw-r--r--"* ]]; then
    check_pass "644"
else
    check_warn "${PERM:-unknown} (建议 644)"
fi

echo -n "[5/6] softbus_server 进程... "
if "$HDC_BIN" -t "$DEVICE_ID" shell 'ps -ef 2>/dev/null || ps aux' | grep -v grep | grep -q softbus_server; then
    check_pass "运行中"
else
...
echo -n "[6/6] 配置条目... "
ENTRY_COUNT=$("$HDC_BIN" -t "$DEVICE_ID" shell "grep -c SESSION_NAME $TARGET_PATH 2>/dev/null" | tr -d '[:space:]')
...
echo "配置预览 (前 20 行):"
echo "----------------------------------------"
"$HDC_BIN" -t "$DEVICE_ID" shell "head -20 $TARGET_PATH"
echo "----------------------------------------"
echo
echo "已配置的 Session 名称:"
"$HDC_BIN" -t "$DEVICE_ID" shell "grep SESSION_NAME $TARGET_PATH" | sed 's/.*"SESSION_NAME"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/  - \1/'
